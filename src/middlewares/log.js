...
function log(options = {}) {
  ...
  return async (ctx, next) => {
    ...
    try {
      ...
      ctx.log.error({ err, event: 'error' }, `Unhandled exception occured on the request: ${ctx.reqId}`);
      throw err;
    }

    ctx.responseTime = new Date() - startTime;
    ctx.log.info({ req: ctx, res: ctx, event: 'response' }, `Request successfully completed for id: ${ctx.reqId}`);
  };
}
